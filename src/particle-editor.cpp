...
ParticleEditor::ParticleEditor( string path ) {
	json::load( [this]( const rapidjson::Value& value ) { return m_pattern.setValue( value ); }, path );
}
...
void ParticleEditor::renderTextureWindow() {
	...
	iterate = [&iterate, this]( std::filesystem::path path ) {
		std::filesystem::recursive_directory_iterator it = std::filesystem::recursive_directory_iterator( path );
		int columns = 0;
		for( const auto directory : it ) {
			if( directory.path().extension() == ".png" ) {
				...
				sf::Sprite sprite;
				sprite.setTexture( Gfx::Sprite::get( Gfx::Sprite::get( directory.path().string() ) ) );
...
void ParticleEditor::save() {
	...
	if( json::save( [this] { return m_pattern.getValue(); }, Folders::Bullets + m_pattern.name + ".bullet" ) )
		Debug::addMessage( Utils::format( "Saved \"%s\" at %s", ( Folders::Bullets + m_pattern.name + ".bullet" ).c_str(), str.c_str() ), DebugType::Info );
