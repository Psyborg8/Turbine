...
rapidjson::Value getValue( const Particle::Pattern& value ) {
	...
	saveValue( value.shape.priority );
...
Particle::Pattern loadPattern( string name ) {
	...
	Debug::addMessage( path );

	FILE* pFile;
	if( fopen_s( &pFile, path.c_str(), "rb" ) ) {
		Debug::addMessage( "Pattern " + path + " doesn't exist.", DebugType::Error );
		return out;
	}
...
Emitter::Pattern loadEmitter( string name ) {
	...
	// Parse JSON;
	Debug::addMessage( "Loading Emitter: " + name, DebugType::Info );

	FILE* pFile;
	if( fopen_s( &pFile, name.c_str(), "rb" ) ) {
		Debug::addMessage( "Emitter " + name + " doesn't exist.", DebugType::Error );
		return out;
	}
	...
	if( document.HasParseError() ) {
		Debug::addMessage( "Emitter \"" + name + "\" has JSON parse error", DebugType::Error );
		return out;
	}
...
void getValue( const rapidjson::Value& json, Particle::Pattern& value ) {
	...
	loadValue( value.shape.priority );
