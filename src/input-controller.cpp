...
void Controller::loadController( sf::Event::JoystickConnectEvent joystick ) {
	const sf::Joystick::Identification id
		= sf::Joystick::getIdentification( joystick.joystickId );
	m_name		= id.name;
...
float Controller::getAxisState( ControllerAxis axis ) {
	return sf::Joystick::getAxisPosition(
		m_id, axisMap.at( sf::Joystick::getIdentification( m_id ).name ).at( axis ) );
}

//--------------------------------------------------------------------------------

ControllerButton Controller::getButton( uint32_t buttonid ) {
	const unordered_map< ControllerButton, uint32_t >& bmap
		= buttonMap.at( sf::Joystick::getIdentification( m_id ).name );
...
ControllerAxis Controller::getAxis( sf::Joystick::Axis axis ) {
	const unordered_map< ControllerAxis, sf::Joystick::Axis >& bmap
		= axisMap.at( sf::Joystick::getIdentification( m_id ).name );
