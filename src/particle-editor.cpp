...
void BulletPatternEditor::renderPattern( Gfx::Particle::Pattern& pattern ) {
	...
	if( ImGui::Button( "Load" ) ) {
		...
		Gfx::Particle::Pattern* pPattern = &pattern;
		m_loadWindowCallback = [pPattern]( string path ) { *pPattern = Gfx::loadPattern( path ); };
		m_loadWindowType = FileBrowserType::Pattern;
	}
	...
	// Priority
	m_changed |= ImGui::InputInt( "Priority", &pattern.shape.priority );
	...
	m_changed |= ImGui::Checkbox( "Attached", &pattern.inheritance.attach );
	...
	m_changed |= ImGui::Checkbox( "Acceleration", &pattern.inheritance.acceleration );
	m_changed |= ImGui::Checkbox( "Origin", &pattern.inheritance.origin );
	m_changed |= ImGui::Checkbox( "Origin Velocity", &pattern.inheritance.originVelocity );
	m_changed |= ImGui::Checkbox( "Rotation", &pattern.inheritance.rotation );
	m_changed |= ImGui::Checkbox( "Spin", &pattern.inheritance.spin );
	m_changed |= ImGui::Checkbox( "Size", &pattern.inheritance.size );
	m_changed |= ImGui::Checkbox( "Color", &pattern.inheritance.color );
...
void BulletPatternEditor::renderEmitter( Gfx::Emitter::Pattern& emitter ) {
	...
	if( ImGui::Button( "Load" ) ) {
		m_loadWindowOpen = true;
		Gfx::Emitter::Pattern* pEmitter = &emitter;
		m_loadWindowCallback = [pEmitter]( string path ) { *pEmitter = Gfx::loadEmitter( path ); };
		m_loadWindowType = FileBrowserType::Emitter;
	}
...
void BulletPatternEditor::renderEmitterTreeNode( Gfx::Emitter::Pattern& emitter, vector< Tab > stack ) {
	...
	if( stack.size() > 1u ) {
		...
		if( ImGui::Button( ( "Remove##" + std::to_string( m_patternTreeID++ ) ).c_str() ) ) {
			...
			const auto it = find_if( m_tabs.begin(), m_tabs.end(),
									 [&emitter]( const Tab& tab ) {
										 return tab.pattern == &emitter;
									 } );
			if( it != m_tabs.end() )
...
void BulletPatternEditor::renderLoadWindow() {
	ImGui::Begin( "Load Pattern", &m_loadWindowOpen, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove );

	namespace fs = std::filesystem;
	function< void( string ) > folder = nullptr;
	folder = [&folder, this]( string directory ) {
		ImGui::PushID( directory.c_str() );

		string extension;
		if( m_loadWindowType == FileBrowserType::Pattern )
			extension = ".bullet";
		else if( m_loadWindowType == FileBrowserType::Emitter )
			extension = ".emitter";
		else
			extension = ".png";

		for( auto& subdirectory : fs::directory_iterator( directory.c_str() ) ) {
			if( subdirectory.is_directory() ) {
				fs::path path = subdirectory.path();
		...
		for( auto& file : fs::directory_iterator( directory.c_str() ) ) {
			fs::path path = file.path();
			if( path.has_extension() && path.extension() == extension.c_str() ) {
				string name = path.filename().string();

				if( ImGui::Button( name.c_str() ) ) {
					m_loadWindowCallback( path.string() );
	...
	string path;
	if( m_loadWindowType == FileBrowserType::Texture )
		path = Folders::Particles;
	else
		path = Folders::Bullets;

	folder( path );

	// Center Window
	ImGui::SetWindowSize( ImVec2( 800.f, 600.f ) );
	ImVec2 pos = ImVec2( float( System::getSystemInfo().width ) / 2.f, float( System::getSystemInfo().height ) / 2.f );
	pos.x -= ImGui::GetWindowSize().x / 2.f;
	pos.y -= ImGui::GetWindowSize().y / 2.f;
