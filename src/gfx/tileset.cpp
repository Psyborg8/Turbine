...
void loadTileset( string name, Math::Vec2 tileSize, string path ) {
	...
	Debug::addMessage( "Loading Tileset: " + name + " - " + path, DebugType::Info );

	if( !std::filesystem::exists( std::filesystem::path( path ) ) ) {
		Debug::addMessage( "Tileset path " + path + " not found", DebugType::Error );
		return;
	}
...
void unloadTileset( string name ) {
	Debug::addMessage( "Unloading Tileset", DebugType::Info );
